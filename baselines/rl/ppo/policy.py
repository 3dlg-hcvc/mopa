...
class MapNetPolicy(NetPolicy):
    ...
    def act(
        ...
        features, rnn_hidden_states, object_maps = self.net(
            observations, rnn_hidden_states, prev_actions, masks, object_maps
        )
        ...
        return value, action, action_log_probs, rnn_hidden_states, object_maps

    def get_value(self, observations, rnn_hidden_states, prev_actions, masks, object_maps):
...
class BaselineNetNonOracleOrig(Net):
    r"""Network which passes the input image through CNN and concatenates
    goal vector with CNN's output and passes that through RNN.
    """
