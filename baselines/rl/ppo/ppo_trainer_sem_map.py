...
class SemMapOnTrainer(BaseRLTrainer):
    r"""Trainer class for semantic map
    """
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            for i in range(self.envs.num_envs):
                ...
                self.object_maps[i, :, :, :] = self.build_map(batch[i], self.object_maps[i, :, :, :], self.grid_map[i,:,:,:])
    ...
    def build_map(self, observations, object_maps, grid_map):
        ...
        semantic = observations['semantic'].squeeze(-1).cpu().numpy()
        theta = observations['episodic_compass'].cpu().numpy()[0]
        ...
        _agent_locs = self.to_grid(location[0], location[1])
        # grid_map[:,:,0] = fog_of_war.reveal_fog_of_war(grid_map[:,:,0], 
        #                                              _agent_locs, 
        #                                              theta)
        # self.fog_of_war_mask = fog_of_war.reveal_fog_of_war(grid_map[:,:,0].astype(np.uint8), 
        #                                             self.fog_of_war_mask.astype(np.uint8),
        #                                             _agent_locs, 
        #                                             theta,
        #                                             fov=self.config.TASK_CONFIG.SIMULATOR.DEPTH_SENSOR.HFOV)
        
        #grid_map[:,:,0] *= self.fog_of_war_mask
        #grid_map[:,:,1] *= self.fog_of_war_mask # don't mask, let it accumulate
        _map = torch.tensor(grid_map)
        object_maps[:_map[:,:,0].shape[0], :_map[:,:,0].shape[1], 0] = _map[:,:,0]
        object_maps[:_map[:,:,1].shape[0], :_map[:,:,1].shape[1], 1] = _map[:,:,1]
        
        # mark agent
        object_maps[:, :, 2] = 0
        object_maps[
         int(max(0, _agent_locs[0] - self.config.TASK_CONFIG.TASK.OBJECT_MAP_SENSOR.object_padding)):
             int(min(_map[:,:,1].shape[0], _agent_locs[0] + self.config.TASK_CONFIG.TASK.OBJECT_MAP_SENSOR.object_padding)),
         int(max(0, _agent_locs[1] - self.config.TASK_CONFIG.TASK.OBJECT_MAP_SENSOR.object_padding)):
             int(min(_map[:,:,1].shape[1], _agent_locs[1] + self.config.TASK_CONFIG.TASK.OBJECT_MAP_SENSOR.object_padding)),
         2] = 10
    ...
    def _add_to_map(self, coords, semantic, grid_map):
        XYZS = np.concatenate((coords, semantic[..., np.newaxis]),axis=-1)
    ...
    def to_grid(self, x, y):
        return np.array([np.round(x/self.map_resolution)+self.map_center[0], 
                np.round(y/self.map_resolution)+self.map_center[1]]).astype(int)
