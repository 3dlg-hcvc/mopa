...
@baseline_registry.register_trainer(name="mapfmmon")
class MapWithFMMOnTrainer(BaseRLTrainer):
    r"""Trainer class for fast marching method
    """
    ...
    def _eval_checkpoint(
        ...
        angles = np.zeros([self.config.NUM_ENVIRONMENTS,1], dtype=np.float)
        ...
        self.grid_map = np.zeros([self.config.NUM_ENVIRONMENTS, self.map_grid_size, self.map_grid_size, 2], dtype=int)
        #self.grid_map[:,:,:,0].fill(-1)
        ...
        self.goal_selem = skimage.morphology.disk(1)
        self.collision_map = np.zeros([self.config.NUM_ENVIRONMENTS, self.map_grid_size, self.map_grid_size], dtype=int)
        self.visited = np.zeros([self.config.NUM_ENVIRONMENTS, self.map_grid_size, self.map_grid_size], dtype=int)
        self.traversible = np.zeros([self.config.NUM_ENVIRONMENTS, self.map_grid_size, self.map_grid_size], dtype=int)
        ...
        self.possible_actions = [HabitatSimActions.MOVE_FORWARD, 
                                 HabitatSimActions.TURN_LEFT,
                                 HabitatSimActions.TURN_RIGHT]
        
        local_goal = np.zeros([self.config.NUM_ENVIRONMENTS, 2], dtype=np.float)
        prev_locs = np.zeros([self.config.NUM_ENVIRONMENTS, 2], dtype=np.int)
        enter_untrap_mode = torch.zeros(
            self.config.NUM_ENVIRONMENTS,
            1,
            device=self.device,
            dtype=torch.long
        )
        untrap_mode_queues = [[] for _ in range(self.config.NUM_ENVIRONMENTS)]
        ...
        while (
            ...
            for i in range(self.envs.num_envs):
                ...
                # 5) Move to the goal
                # Path Planner
                if (enter_untrap_mode[i] == 1 and
                    self.get_l2_distance(agent_locs[i][0], prev_locs[i][0], agent_locs[i][1], prev_locs[i][1])
                        < 1.5):
                    # Untrap until agent location changes
                    if len(untrap_mode_queues[i]) == 0:
                        untrap_mode_queues[i].append(1)
                        untrap_mode_queues[i].append(2)
                        
                    _dir = untrap_mode_queues[i].pop(0)
                    if _dir == 1:
                        actions[i] = HabitatSimActions.TURN_LEFT
                    elif _dir == 2:
                        actions[i] = HabitatSimActions.MOVE_FORWARD
                    untrap_mode_queues[i].append(_dir)
                else:
                    enter_untrap_mode[i] = 0 # come out of untrap mode
                    loc_r, loc_c = agent_locs[i]
                    local_w = local_h = self.config.RL.POLICY.planning_map_size
                    full_w = full_h = self.map_grid_size
                    
                    # planning window
                    if self.config.RL.POLICY.planning_use_local:
                    ...
                    self.visited[i, agent_locs[i,0], agent_locs[i,1]] = 1
                    
                    stg, stop, traversible = self._get_stg(
                                                        #self.collision_map[i,:,:],
                                                        self.object_maps[i,:,:,0].cpu().numpy(), 
                                                        agent_locs[i], 
                                                        self.object_maps[i,:,:,3].cpu().numpy(), 
                                                        [gx1, gx2, gy1, gy2],
                                                        #self.object_maps[i,:,:,0].cpu().numpy(), 
                                                        self.collision_map[i,:,:],
                                                        self.visited[i,:,:])
                    local_goal[i] = stg
                    self.traversible[i] = traversible

                    # Deterministic Local Policy
                    if stop == 1:
                        actions[i] = HabitatSimActions.STOP  # Stop
                    else:
                        (stg_x, stg_y) = stg
                        angle_st_goal = math.degrees(math.atan2(stg_x - agent_locs[i, 0],
                                                                stg_y - agent_locs[i, 1]))
                        angle_agent = (np.rad2deg(batch[i]["episodic_compass"].cpu().numpy())) % 360.0
                        if angle_agent > 180:
                            angle_agent -= 360
                        ...
                        angles[i] = relative_angle
                        if relative_angle > self.config.TASK_CONFIG.SIMULATOR.TURN_ANGLE / 2.:
                            actions[i] = HabitatSimActions.TURN_RIGHT
                        elif relative_angle < -self.config.TASK_CONFIG.SIMULATOR.TURN_ANGLE / 2.:
                            actions[i] = HabitatSimActions.TURN_LEFT
                        else:
                            actions[i] = HabitatSimActions.MOVE_FORWARD
                #
            
                prev_locs[i] = agent_locs[i]
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    if len(self.config.VIDEO_OPTION) > 0:
                        frame = observations_to_image(
                                ...
                                traversible=self.traversible[i], 
                        ...
                        if self.config.VIDEO_RENDER_ALL_INFO:
                            ...
                            _m["angle"] = angles[i].item()
                            _m["agent_loc"] = str(agent_locs[i])
                            _m["local_goal"] = str(local_goal[i])
                            _m["global_goal"] = str(goal_grid[i].cpu().numpy())
                            _m["next_goal"] = multion_maps.MULTION_CYL_OBJECT_MAP[next_goal_category[i].item()]
                            _m["action"] = actions[i].item()
                            if self.config.RL.POLICY.EXPLORATION_STRATEGY == "stubborn":
                    ...
                    self.grid_map[i,:,:,:] = np.zeros([self.map_grid_size, self.map_grid_size, 2], dtype=int)
                    #self.grid_map[i,:,:,:].fill(-1)
                    local_goal[i] = np.zeros(
                        2,
                        dtype=np.float
                    )
                    self.collision_map[i,:,:] = np.zeros([self.map_grid_size, self.map_grid_size], dtype=int)
                    self.visited[i,:,:] = np.zeros([self.map_grid_size, self.map_grid_size], dtype=int)
                    self.traversible[i,:,:] = np.zeros([self.map_grid_size, self.map_grid_size], dtype=int)
                    prev_locs[i] = np.zeros([2], dtype=np.int)
                    untrap_mode_queues[i] = []
                    enter_untrap_mode[i] = 0
                ...
                # episode continues
                    ...
                    curr_locs = self.to_grid(batch[i]["episodic_gps"].cpu().numpy())
                    if (infos[i]["collisions"]["is_collision"] or 
                            (actions[i] == HabitatSimActions.MOVE_FORWARD and 
                             self.get_l2_distance(curr_locs[0], prev_locs[i][0], curr_locs[1], prev_locs[i][1])
                            < 1.5)):
                        collision_threshold_steps[i] += 1
                        # self.object_maps[i, :, :, 0] = self.update_collision_map(
                        #                                     agent_locs[i], 
                        #                                     int(angles[i].item()), 
                        #                                     self.object_maps[i, :, :, 0])
                        #self.object_maps[i, agent_locs[i,0], agent_locs[i,1], 0] = 1   #mark collision
                        self.visited[i, curr_locs[0], curr_locs[1]] = 0
                        #self.object_maps[i, int(local_goal[i,0]), int(local_goal[i,1]), 0] = 1   #mark local goal as collision
                        
                        # self.collision_map[i, 
                        #                    agent_locs[i,0]-1:agent_locs[i,0]+1, 
                        #                    agent_locs[i,1]-1:agent_locs[i,1]+1] = 1   #mark collision
                        self.collision_map[i, 
                                           curr_locs[0]-1:curr_locs[0]+1, 
                                           curr_locs[1]-1:curr_locs[1]+1] = 1   #mark collision
                        # curr_locs = self.to_grid(batch[i]["episodic_gps"].cpu().numpy())
                        # self.object_maps[i, curr_locs[0], curr_locs[1], 0] = 1   #mark collision
                        
                        if collision_threshold_steps[i] >= self.config.RL.POLICY.collision_threshold:
                            enter_untrap_mode[i] = 1
                    else:
                        collision_threshold_steps[i] = 0
                    ...
                    if ((actions[i].item() == 0 and infos[i]["sub_success"] == 1) or
                            ...
                            ((self.config.RL.POLICY.EXPLORATION_STRATEGY == "" or self.config.RL.POLICY.EXPLORATION_STRATEGY == "random") 
                                ...
                                (infos[i]["collisions"]["is_collision"] and
                                    collision_threshold_steps[i].item() > self.config.RL.POLICY.collision_threshold) )) or
                            (self.config.RL.POLICY.EXPLORATION_STRATEGY == "stubborn" and 
                                (infos[i]["collisions"]["is_collision"] and
                                    collision_threshold_steps[i].item() > self.config.RL.POLICY.collision_threshold) 
    ...
    def build_map(self, observations, object_maps, grid_map):
        ...
        object_maps[:, :, :, 0] = torch.max(object_maps[:, :, :, 0], torch.tensor(grid_map[:,:,:,0], device='cuda')) #retaining obstacle info
        object_maps[:, :, :, 1] = torch.tensor(grid_map[:,:,:,1], device='cuda')
    ...
    def bfs(self, start, inflation, grid, goal):
        # start -> agent location, inflation -> for obstacles
        h = w = self.map_grid_size
        
        queue = deque([[tuple(start)]])
        seen = set([tuple(start)])
        obs_grid = self.obstacle_inflation(inflation, grid[:,:,0].cpu().numpy())
        #obs_grid = grid[:,:,0].cpu().numpy()
        while queue:
            path = queue.popleft()
            x, y = path[-1]
            ...
            for x2, y2 in adjacents:
                if 0 <= x2 < w and 0 <= y2 < h and (x2, y2) not in seen and obs_grid[x2][y2] != 1:
    ...
    #     for i in range(inflation, -1, -1):
    #         queue = deque([[tuple(start)]])
    #         seen = set([tuple(start)])
    #         obs_grid = self.obstacle_inflation(i, grid[:,:,0].cpu().numpy())
    #         #obs_grid = grid[:,:,0].cpu().numpy()
    #         while queue:
    #             path = queue.popleft()
    #             x, y = path[-1]
    ...
    def obstacle_inflation(self, i, obs_grid):
        grid = np.copy(obs_grid)
        # horizontal_translation
        for j in range(1, i + 1):
            grid = np.maximum(grid, np.pad(obs_grid, ((j, 0), (0, 0)), mode='constant')[:-j, :])
            grid = np.maximum(grid, np.pad(obs_grid, ((0, j), (0, 0)), mode='constant')[j:, :])
            grid = np.maximum(grid, np.pad(obs_grid, ((0, 0), (j, 0)), mode='constant')[:, :-j])
            grid = np.maximum(grid, np.pad(obs_grid, ((0, 0), (0, j)), mode='constant')[:, j:])
            grid = np.maximum(grid, np.pad(obs_grid, ((j, 0), (j, 0)), mode='constant')[:-j, :-j])
            grid = np.maximum(grid, np.pad(obs_grid, ((j, 0), (0, j)), mode='constant')[:-j, j:])
            grid = np.maximum(grid, np.pad(obs_grid, ((0, j), (j, 0)), mode='constant')[j:, :-j])
            grid = np.maximum(grid, np.pad(obs_grid, ((0, j), (0, j)), mode='constant')[j:, j:])
        return grid
    
    def update_collision_map(self, start, agent_theta, objects_map):
        # Rescale angle
        #agent_theta = int(round(np.rad2deg(self.prev_loc[2])))

        # Map agent position into grid
        collision_cell = None

        if agent_theta == 0 or agent_theta == 360:
            collision_cell = [start[1], start[0] + 1]
        elif (agent_theta == 30 or agent_theta == 60):
            collision_cell = [start[1] + 1, start[0] + 1]
        elif agent_theta == 90:
            collision_cell = [start[1] + 1, start[0]]
        elif (agent_theta == 120 or agent_theta == 150):
            collision_cell = [start[1] + 1, start[0] - 1]
        elif agent_theta == 180:
            collision_cell = [start[1], start[0] - 1]
        elif (agent_theta == 210 or agent_theta == 240):
            collision_cell = [start[1] - 1, start[0] - 1]
        elif agent_theta == 270:
            collision_cell = [start[1] - 1, start[0]]
        elif (agent_theta == 300 or agent_theta == 330):
            collision_cell = [start[1] - 1, start[0] + 1]

        objects_map[collision_cell[0]][collision_cell[1]] = 1
        
        return objects_map
    
    def _get_stg(self, grid, start, goal, planning_window, collision_map, visited):
        """Get short-term goal"""
        ...
        # traversible[collision_map[gx1:gx2, gy1:gy2]
        #             [x1:x2, y1:y2] == 1] = 0
        traversible[visited[gx1:gx2, gy1:gy2][x1:x2, y1:y2] == 1] = 1
        ...
        #     #nav_area = traversible[start[0]-1:start[0]+2, start[1]-1:start[1]+2]
        #     for x in range(start[0]-1, start[0]+2, 1):
        #         for y in range(start[1]-1, start[1]+2, 1):
        #             if traversible[x,y]:
        #                 return (x, y), 0, traversible

        traversible[int(start[0] - x1) - 1:int(start[0] - x1) + 1,
                    int(start[1] - y1) - 1:int(start[1] - y1) + 1] = 1

        planner = FMMPlanner(traversible, step_size=2)
        goal = skimage.morphology.binary_dilation(
            goal, self.goal_selem) != True
        goal = 1 - goal * 1.
        ...
        return (stg_x, stg_y), stop, traversible
