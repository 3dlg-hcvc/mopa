...
@baseline_registry.register_trainer(name="semmapfrontier")
class SemMapOnFrontierTrainer(BaseRLTrainer):
    r"""Trainer class for predicted semantic map with frontier exploration
    """
            ...
            for i in range(self.envs.num_envs):
                ...
                if prev_actions[i].item() == 0:
                    ...
                    if len(goal_grid_loc) == 0:
                        # explore
                        frontier_pnts = self.findFrontier(self.object_maps[i, :, :, 0].cpu().numpy())
                ...
                if not not_done_masks[i].item():
                    ...
                    if len(self.config.VIDEO_OPTION) > 0:
                        frame = observations_to_image(
                                ...
                                agent_angle=observations[i]['episodic_compass']
                        )
    ...
    def findFrontier(self, mat):
        frontier_pnts = []
        # frontier_mat = np.zeros(np.shape(mat),dtype=np.uint8).tolist()
        dx = [0,-1,-1,-1,0,1,1,1]
        dy = [1,1,0,-1,-1,-1,0,1]

        # mat = multion_maps.OBJECT_MAP_COLORS[mat]
        ...
        kernel = np.ones((5,5), np.uint8)*1 #255
