...
@baseline_registry.register_trainer(name="mapppon")
class MapWithPathPlannerOnTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            for i in range(self.envs.num_envs):
                ...
                # 5) Move to the goal
                # Path Planner
                dist_to_goal = np.linalg.norm(agent_locs[i] - goal_grid[i].cpu().numpy())
                if dist_to_goal < 2.0:
                    actions[i] = HabitatSimActions.STOP
                else:
                    if enter_untrap_mode[i] == 1:
                        ...
                        path, obs_grid = self.bfs(agent_locs[i], 1, self.object_maps[i,:,:,0], goal_grid[i].cpu().numpy())
                        """ if path is None or len(path) > 50 or len(path) < 10:
                            path = self.bfs(agent_locs[i], 0, self.object_maps[i], goal_grid[i].cpu().numpy()) """
                        ...
                        if path is None:
                            ...
                            enter_untrap_mode[i] = 0    # leave untrap mode
                            if len(path) > 5:
                                local_goal[i] = path[5]
                            else:
                                local_goal[i] = path[-1]
                                
                            ag_theta = np.rad2deg(batch[i]["episodic_compass"].cpu().numpy())
                            goal_angle = math.atan2(local_goal[i,0] - agent_locs[i,0], local_goal[i,1] - agent_locs[i,1])
                            goal_angle = np.rad2deg(goal_angle) % 360
                            goal_angle -= np.round(ag_theta)
                            goal_angle = goal_angle % 360
                            angles[i] = goal_angle[0]
                            
                            if goal_angle <= 5.0 or goal_angle > 355.0:
                                actions[i] = HabitatSimActions.MOVE_FORWARD
                            elif goal_angle < 180.0:
                                actions[i] = HabitatSimActions.TURN_LEFT
                            else:
            ...
            for i in range(n_envs):
                ...
                # episode continues
                    ...
                    if self.config.RL.POLICY.EXPLORATION_STRATEGY == "stubborn" and infos[i]["collisions"]["is_collision"]:
                        ...
                        # self.object_maps[i, curr_locs[0], curr_locs[1], 0] = 1   #mark as obstacle on collision
    ...
    def bfs(self, start, inflation, grid, goal):
        ...
        while queue:
            ...
            for x2, y2 in adjacents:
                if 0 <= x2 < w and 0 <= y2 < h and (x2, y2) not in seen and grid[x2][y2] != 1:
