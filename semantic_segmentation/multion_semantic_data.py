...
class MultionSemanticData(Dataset):
    def __init__(self, transform=None, phase_train=True, training_data=None):

        self.phase_train = phase_train
        self.transform = transform

        try:
            self.training_data = h5py.File(training_data, mode='r', libver='latest')
        except:
            print('Check file at ',training_data)
            self.training_data = None

    def __len__(self):
        if self.phase_train:
            return len(self.training_data['rgb'])
        else:
    ...
    def __getitem__(self, idx):
        ...
        # label = imageio.imread(self.training_data['semantic'][idx])
        # depth = imageio.imread(self.training_data['depth'][idx])
        # image = imageio.imread(self.training_data['rgb'][idx])
        
        label = self.training_data['semantic'][idx]
        depth = self.training_data['depth'][idx]
        image = self.training_data['rgb'][idx]
...
class scaleNorm(object):
    def __call__(self, sample, image_h=256, image_w=256):
